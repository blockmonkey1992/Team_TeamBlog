...
function LandingPage(props) {
    ...
    //Category Componet에서 받아온 카테고리 값 설정해주기
    ...
    useEffect(() => {
        ...
        //더보기 버튼을 누르지 않았을 때 기본적으로 보여지는 Cards들의 개수를 구하고 이후 개수만큼 Cards를 렌더링
        let body = {
            skip : Skip,
            limit : Limit,
        }
        ...
        //카테고리 버튼을 누를 때마다 해당 카테고리에 속하는 Cards를 다시 렌더링하도록 함.
        Axios.get(`/api/post/category/${CategoryValue}`)
            .then( response => {
                //해당 카테고에 속하는 Cards 값이 0개 이상이면 Cards를 보여주기
    ...
    const getCards = (body) => {
        ...
        .then(response => {
            if(response.data.success){
                //기본적으로 8개의 Cards를 보여주고, 이후 더보기 버튼을 눌렀을 때 8개의 Cards를 더 보여줌.
                if(body.loadMore){
                    setData([...Data, ...response.data.result]);
                } else {
                    setData(response.data.result);
                }
                //보여줄 수 있는 Cards들이 존재하지 않으면 마지막 페이지임을 알려줌.
